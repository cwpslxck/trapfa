...
function ArtistPart({ image, artistName, role, url }) {
  return (
    ...
    >
      ...
      <div className="p-2 absolute inset-0 h-full w-full bg-gradient-to-b from-black/0 via-black/10 to-black/50 hover:from-black/10 hover:via-black/20 hover:to-black/50 transition-all">
        <div className="flex flex-col justify-between items-center h-full w-full">
          {role ? (
            <div className="py-1 px-3 rounded-full bg-white/10 backdrop-blur-lg">
              {role}
            </div>
          ) : (
            <div></div>
          )}
          <h2 className="font-bold leading-snug text-2xl uppercase tracking-wide">
            {artistName}
          </h2>
        </div>
      </div>
